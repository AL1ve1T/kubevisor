...
Installs the full KubeTopo stack: the **backend** (graph API + OTLP/HTTP
...
Collector** that feeds it. Every resource is placed in the `kubetopo` namespace
(`namespace.name`).
...
# The chart creates and owns the kubetopo namespace:
helm install kubetopo ./kubetopo

# Render without installing (inspect the generated manifests):
helm template kubetopo ./kubetopo

# Upgrade with overrides:
helm upgrade kubetopo ./kubetopo \
...
> By default the chart **creates** the `kubetopo` namespace (`namespace.create`)
> and pins every resource to it (`namespace.name`). If you prefer Helm itself to
> own the install namespace, install with
> `-n kubetopo --create-namespace --set namespace.create=false`.
...
| `fullnameOverride` | `"kubetopo"` | Override the resource-name prefix (else the release name). |
| `imagePullSecrets` | `[]` | Pull secrets applied to every pod. |
| `namespace.name` | `kubetopo` | Namespace all resources are placed in (empty → release namespace). |
...
| `backend.image.repository` | `kubetopo-backend` | Backend image repo. |
...
| `backend.otlp.httpPort` | `4318` | OTLP/HTTP ingestion port (`KUBETOPO_OTLP_HTTP_PORT`). `0` or `== api.port` serves ingestion on the main port only. |
...
| `backend.staleThresholdSeconds` | `120` | Idle node/edge cleanup window (`KUBETOPO_STALE_THRESHOLD_SECONDS`). |
| `backend.extraEnv` | `[]` | Extra env for advanced `kubetopo.*` tuning (thresholds, memory-limit-bytes, etc.). |
...
| `frontend.image.repository` | `kubetopo-frontend` | Frontend image repo. |
...
| `postgres.auth.database` | `kubetopo` | Database name. |
| `postgres.auth.username` | `kubetopo` | Database user. |
...
| `externalDatabase.database` | `kubetopo` | DB name. |
...
helm upgrade kubetopo ./kubetopo --reuse-values \
...
- **Ingress** — the UI and API are `ClusterIP`; use `kubectl port-forward` or add
  your own Ingress in front of the `kubetopo-frontend` Service.
...
| --- | --- | --- |
| Backend | `backend/` | `kubetopo-backend:<appVersion>` |
| Frontend | `frontend/` | `kubetopo-frontend:<appVersion>` |
