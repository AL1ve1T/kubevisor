...
import com.kubevisor.model.Node;
import com.kubevisor.model.NodeType;
import com.kubevisor.model.PodInstance;
...
public class GraphStateManager {
    ...
    public GraphStateManager(KubevisorProperties properties) {
    ...
    public void registerNode(String serviceName, String namespace) {
        ...
        Node node = nodes.computeIfAbsent(serviceName,
                id -> {
                    dirty.set(true);
                    return new Node(id, id, com.kubevisor.model.NodeType.SERVICE, namespace);
                });
        node.touch();
    }
    ...
    private void upgradeNodeTypeIfMoreSpecific(Node node, NodeType candidateType) {
        if (node.getType() == com.kubevisor.model.NodeType.SERVICE
                && candidateType != com.kubevisor.model.NodeType.SERVICE
                && candidateType != com.kubevisor.model.NodeType.INPUT) {
