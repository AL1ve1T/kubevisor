...
public class GraphStateManager {
    ...
    public GraphStateManager(KubetopoProperties properties) {
    ...
    public void registerNode(String serviceName, String namespace) {
        ...
        Node node = nodes.computeIfAbsent(serviceName,
                id -> {
                    dirty.set(true);
                    return new Node(id, id, com.kubetopo.model.NodeType.SERVICE, namespace);
                });
        node.touch();
    }
    ...
    private void upgradeNodeTypeIfMoreSpecific(Node node, NodeType candidateType) {
        if (node.getType() == com.kubetopo.model.NodeType.SERVICE
                && candidateType != com.kubetopo.model.NodeType.SERVICE
                && candidateType != com.kubetopo.model.NodeType.INPUT) {
