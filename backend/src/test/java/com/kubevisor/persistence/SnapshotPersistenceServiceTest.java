...
@DataJpaTest
@Import({ SnapshotPersistenceService.class, KubevisorProperties.class,
...
class SnapshotPersistenceServiceTest {
    ...
    void save_persistsSnapshotToDatabase() {
        GraphSnapshot snapshot = new GraphSnapshot(
                "default",
                List.of(new GraphSnapshot.NodeDto("svc-a", "svc-a", NodeType.SERVICE,
                        com.kubevisor.model.PodPhase.UNKNOWN, 0, 0, null, null, Instant.now(), List.of())),
                List.of(),
                Instant.now());
