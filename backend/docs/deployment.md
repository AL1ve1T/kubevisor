...
it copies the built `target/kubetopo-backend-*.jar`, exposes `8080` (graph API)
...
| File | Contents |
| --- | --- |
| `namespace.yml` | The `kubetopo` namespace. |
| `deployment.yml` | `kubetopo-backend` Deployment (profile `k8s`, `DB_*` from a Secret, `/actuator/health` probes; exposes `8080` + OTLP `4318`). |
...
- `DB_USERNAME` / `DB_PASSWORD` come from the `kubetopo-postgres-secret` Secret.
- `KUBETOPO_STALE_THRESHOLD_SECONDS` overrides the stale cleanup window.
- `KUBETOPO_OTLP_HTTP_PORT` (default `4318`) sets the OTLP/HTTP ingestion port —
...
- **Exporter:** `otlphttp/kubetopo_backend` with `encoding: json`, endpoint from
  the `KUBETOPO_BACKEND_ENDPOINT` env var (the backend's OTLP/HTTP port `4318`).
  - Backend on the host (local dev): `http://host.docker.internal:4318`.
  - Backend in-cluster: `http://kubetopo-backend.kubetopo.svc.cluster.local:4318`.
...
- `k8s.container.memory_limit_utilization` is intentionally **not** enabled on
  ...
  divided by `kubetopo.memory-limit-bytes`.
...
- If the backend runs locally (not in-cluster), point the collector's
  `KUBETOPO_BACKEND_ENDPOINT` at the host and restart the collector.
...
docker build -t kubetopo-backend:latest .
...
collector toggle) and places everything in the `kubetopo` namespace.

```bash
# The chart creates and owns the kubetopo namespace:
helm install kubetopo ./helm/kubetopo

helm template kubetopo ./helm/kubetopo                 # render only
helm upgrade  kubetopo ./helm/kubetopo --set backend.image.tag=0.2.0
```

- All resources land in `namespace.name` (default `kubetopo`); the chart creates
...
- The **frontend** (`frontend.enabled`) is an nginx image that serves the SPA and
  ...
  `kubetopo-frontend`. Disable with `frontend.enabled=false`.
...
See [helm/kubetopo/README.md](../helm/kubetopo/README.md) for the full values
reference.
